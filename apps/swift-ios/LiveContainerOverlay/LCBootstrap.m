...
static void t3LaunchLog(NSString *message) {
    NSLog(@"[T3-launch] %@", message);
    const char *homePathC = getenv("LC_HOME_PATH");
    if (!homePathC) homePathC = getenv("HOME");
    if (!homePathC) return;
    NSString *docPath = [[NSString stringWithUTF8String:homePathC] stringByAppendingPathComponent:@"Documents"];
    NSString *line = [NSString stringWithFormat:@"%@ %@\n", NSDate.date, message];
    NSString *logPath = [docPath stringByAppendingPathComponent:@"t3-live-import-log.txt"];
...
static void dumpT3Diagnostics(void) {
    NSMutableString *report = [NSMutableString string];
    [report appendFormat:@"timestamp: %@\n", NSDate.date];
    [report appendFormat:@"bundle: %@\n", NSBundle.mainBundle.bundleIdentifier ?: @"(nil)"];
    [report appendFormat:@"HOME: %s\n", getenv("HOME") ?: "(null)"];
    [report appendFormat:@"LC_HOME_PATH: %s\n", getenv("LC_HOME_PATH") ?: "(null)"];
    [report appendFormat:@"appGroupID: %@\n", [LCSharedUtils appGroupID] ?: @"(nil)"];
    [report appendFormat:@"teamIdentifier: %@\n", [LCSharedUtils teamIdentifier] ?: @"(nil)"];

    NSUserDefaults *std = NSUserDefaults.standardUserDefaults;
    NSUserDefaults *shared = NSUserDefaults.lcSharedDefaults;
    NSData *stdData = [std objectForKey:@"LCCertificateData"];
    NSString *stdPassword = [std objectForKey:@"LCCertificatePassword"];
    NSData *sharedData = [shared objectForKey:@"LCCertificateData"];
    NSString *sharedPassword = [shared objectForKey:@"LCCertificatePassword"];
    [report appendFormat:@"standard defaults: data %@, password %@\n", stdData ? [NSString stringWithFormat:@"%lu bytes", (unsigned long)stdData.length] : @"(nil)", stdPassword ? [NSString stringWithFormat:@"%lu chars", (unsigned long)stdPassword.length] : @"(nil)"];
    [report appendFormat:@"shared defaults (%@): data %@, password %@\n", [LCSharedUtils appGroupID] ?: @"(nil)", sharedData ? [NSString stringWithFormat:@"%lu bytes", (unsigned long)sharedData.length] : @"(nil)", sharedPassword ? [NSString stringWithFormat:@"%lu chars", (unsigned long)sharedPassword.length] : @"(nil)"];
    [report appendFormat:@"selected: %@\n", [std objectForKey:@"selected"] ?: @"(nil)"];
    [report appendFormat:@"selectedContainer: %@\n", [std objectForKey:@"selectedContainer"] ?: @"(nil)"];

    // Host entitlements
    NSString *entitlementXML = getExecutableEntitlementXML(NSBundle.mainBundle.executablePath ?: @"");
    [report appendFormat:@"host entitlements:\n%@\n", entitlementXML ?: @"(none)"];

    NSLog(@"[T3] DIAGNOSTICS\n%@", report);

    NSString *homePath = nil;
    const char *homePathC = getenv("LC_HOME_PATH");
    if (!homePathC) {
        homePathC = getenv("HOME");
    }
    if (homePathC) {
        homePath = [NSString stringWithUTF8String:homePathC];
    }
    NSString *docPath = homePath ? [homePath stringByAppendingPathComponent:@"Documents"] : nil;
    if (docPath) {
        NSError *writeError = nil;
        BOOL ok = [report writeToFile:[docPath stringByAppendingPathComponent:@"t3-live-diagnose.txt"] atomically:YES encoding:NSUTF8StringEncoding error:&writeError];
        NSLog(@"[T3] diagnostics file written: %@ error: %@", ok ? @"yes" : @"no", writeError ?: @"(none)");
    }
}
...
static NSString* invokeAppMain(NSString *selectedApp, NSString *selectedContainer, int argc, char *argv[]) {
    ...
    t3LaunchLog([NSString stringWithFormat:@"invokeAppMain entry: selected=%@ container=%@ certPassword=%@", selectedApp ?: @"(nil)", selectedContainer ?: @"(nil)", LCSharedUtils.certificatePassword ? @"yes" : @"no"]);
    ...
    if (!LCSharedUtils.certificatePassword && !isSideStore) {
        ...
        if(@available(iOS 26.0 ,*))  {
            dumpT3Diagnostics();
            NSString *passwordInStandardDefaults = [NSUserDefaults.standardUserDefaults objectForKey:@"LCCertificatePassword"];
            NSString *passwordInSharedDefaults = [NSUserDefaults.lcSharedDefaults objectForKey:@"LCCertificatePassword"];
            NSData *dataInStandardDefaults = [NSUserDefaults.standardUserDefaults objectForKey:@"LCCertificateData"];
            NSString *selectedValue = [NSUserDefaults.standardUserDefaults objectForKey:@"selected"];
            return [NSString stringWithFormat:@"JITLess mode is required since iOS 26 [t3-live-r10]. No signing certificate found.\nApp Group: %@\npassword in app defaults: %@\npassword in shared defaults: %@\ncert data in app defaults: %@\nselected: %@\nDiagnostics were written to Documents/t3-live-diagnose.txt (visible in the Files app).\nIn T3 Code Live, run an iPhone app to trigger the certificate import from SideStore, approve it, then run again.", [LCSharedUtils appGroupID], passwordInStandardDefaults ? @"yes" : @"no", passwordInSharedDefaults ? @"yes" : @"no", dataInStandardDefaults ? [NSString stringWithFormat:@"%lu bytes", (unsigned long)dataInStandardDefaults.length] : @"no", selectedValue ?: @"(nil)"];
        }
#endif
    ...
    *_CFGetProgname() = NSProcessInfo.processInfo.processName.UTF8String;
    ...
    if (!appHandle || (uint64_t)appHandle > 0xf00000000000) {
        ...
        t3LaunchLog([NSString stringWithFormat:@"guest dlopen FAILED: %@", appError]);
        NSLog(@"[LCBootstrap] %@", appError);
        *path = oldPath;
        return appError;
    }
    t3LaunchLog(@"guest executable dlopened, loading bundle");
    ...
    t3LaunchLog([NSString stringWithFormat:@"jumping to guest main %p (exec=%@)", appMain, [NSString stringWithUTF8String:appExecPath] ?: @"(nil)"]);
    int ret;
    if(!is32bit) {
    ...
    t3LaunchLog([NSString stringWithFormat:@"guest main returned with code %d", ret]);
...
static void exceptionHandler(NSException *exception) {
    ...
    t3LaunchLog([NSString stringWithFormat:@"UNCAUGHT EXCEPTION: %@", error]);
    if(isLiveProcess) {
