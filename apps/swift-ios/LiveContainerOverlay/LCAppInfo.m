...
- (NSDictionary *)generateWebClipConfigWithContainerId:(NSString*)containerId iconStyle:(GeneratedIconStyle)style{
    NSString* appClipUrl;
    if(containerId) {
        appClipUrl = [NSString stringWithFormat:@"t3code-livecontainer://livecontainer-launch?bundle-name=%@&container-folder-name=%@", self.bundlePath.lastPathComponent, containerId];
    } else {
        appClipUrl = [NSString stringWithFormat:@"t3code-livecontainer://livecontainer-launch?bundle-name=%@", self.bundlePath.lastPathComponent];
    }
            ...
            void (^signCompletionHandler)(BOOL success, NSError *error)  = ^(BOOL success, NSError *_Nullable error) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    if(!success) {
                        ...
                        bool signatureValid = checkCodeSignatureWithError(executablePath.UTF8String, messageBuffer, sizeof(messageBuffer));
                        if(signatureValid) {
                            ...
                            NSLog(@"[LC] signature check failed: %@", detailed);
                            completetionHandler(NO, detailed);
                        }
                    }
