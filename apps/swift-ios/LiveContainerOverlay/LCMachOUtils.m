...
bool checkCodeSignature(const char* path) {
    ...
    return checkCodeSignatureWithError(path, messageBuffer, sizeof(messageBuffer));
}

bool checkCodeSignatureWithError(const char* path, char* errorMessageOut, size_t errorMessageSize) {
    if (errorMessageOut && errorMessageSize > 0) {
        errorMessageOut[0] = '\0';
    }
    ...
    LCParseMachO(path, true, ^(const char *path, struct mach_header_64 *header, int fd, void *filePtr) {
        ...
        if(!codeSignatureCommand) {
            if (errorMessageOut && errorMessageSize > 0) {
                snprintf(errorMessageOut, errorMessageSize, "no LC_CODE_SIGNATURE command in Mach-O");
            }
            return;
        }
        ...
        if ( addFileSigsReault == -1 ) {
            if (errorMessageOut && errorMessageSize > 0) {
                snprintf(errorMessageOut, errorMessageSize, "F_ADDFILESIGS_RETURN failed: %s (errno %d)", strerror(errno), errno);
            }
            ans = false;
            return;
        }
        ...
        if (checkLVresult == 0) {
            ...
            if (errorMessageOut && errorMessageSize > 0) {
                if (messageBuffer[0] != '\0') {
                    snprintf(errorMessageOut, errorMessageSize, "F_CHECK_LV failed (errno %d): %s", errno, messageBuffer);
                } else {
                    snprintf(errorMessageOut, errorMessageSize, "F_CHECK_LV failed (errno %d)", errno);
                }
            }
            ans = false;
            return;
    ...
    if (!ans && errorMessageOut && errorMessageOut[0] == '\0') {
        snprintf(errorMessageOut, errorMessageSize, "no valid arm64 slice with code signature found");
    }
    return ans;
}
